...
function ManualPicker({ type, value, setValue, fixedWidth }: ManualPickerProps) {
    return <div className={`${styles.priceManualPicker} ${fixedWidth ? styles.fixed : ""}`}>
        ...
        <div>
            {
                value !== false && <div className={styles.input}>
                    <p>R$</p>
                    <input
                        type="number"
                        name={type}
            ...
            <DashboardCheckbox checked={value !== false} onCheckedChange={checked => checked === false ? setValue?.(false) : setValue?.(type === "min" ? DEFAULT_MIN_VALUE : DEFAULT_MAX_VALUE)} />
        </div>
    </div>
}
...
export default function DashboardPricePicker({ defaultValues, fixedWidth = false }: { defaultValues?: { min?: number, max?: number }, fixedWidth?: boolean }) {
    ...
    ] : minValue && maxValue == false ? [minValue] : maxValue && minValue == false ? [maxValue] : [DEFAULT_MIN_VALUE, DEFAULT_MAX_VALUE]
